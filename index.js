...
async function askHowl(userMessage, extraContext) {
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: HOWL_SYSTEM + (extraContext ? '\n\n' + extraContext : '') },
...
async function sendDailyCheckin() {
  ...
  const context = `Afternoon check-in. Balance last 7 days: Grow=${balance.grow}, Create=${balance.create}, Nourish=${balance.nourish}. Most neglected: ${CHARTER_NAMES[neglected]}. Ask about energy level dramatically.`;
...
bot.on('message', async (msg) => {
  ...
  try {
    if (text === '/start') {
      const intro = await askHowl('first meeting', 'Introduce yourself as her productivity companion. Mention 3 charters briefly. Tell her you check in at 1:30pm daily. Be dramatic and charming. Under 150 words.');
    ...
    if (state.awaitingAddCharter && !['🧠 Grow & Build', '✍️ Create & Express', '🌿 Slow & Nourish'].includes(text)) {
      ...
      state.tasks[charter].push({ id: `${charter[0]}${Date.now()}`, text, done: false });
      state.awaitingAddCharter = null;
      const response = await askHowl(`task added: "${text}" to ${CHARTER_NAMES[charter]}`, 'Confirm dramatically.');
      await bot.sendMessage(chatId, response);
      return;
    }

    if (text === '🧠 Grow & Build' && state.awaitingAdd) { state.awaitingAdd = false; state.awaitingAddCharter = 'grow'; await bot.sendMessage(chatId, 'Splendid! What magnificent task shall I add to Grow & Build?'); return; }
    if (text === '✍️ Create & Express' && state.awaitingAdd) { state.awaitingAdd = false; state.awaitingAddCharter = 'create'; await bot.sendMessage(chatId, 'Delightful. What shall I add to Create & Express?'); return; }
    if (text === '🌿 Slow & Nourish' && state.awaitingAdd) { state.awaitingAdd = false; state.awaitingAddCharter = 'nourish'; await bot.sendMessage(chatId, 'Restorative choice. What shall I add to Slow & Nourish?'); return; }
    ...
    if (energyMap[text]) {
      ...
      const context = `Energy: ${energy}. Balance: Grow=${balance.grow}, Create=${balance.create}, Nourish=${balance.nourish}. Present menu theatrically. Tell her to reply 1, 2, 3 or combo.`;
      const response = await askHowl(`energy is ${energy}`, context);
      await bot.sendMessage(chatId, response + '\n\n' + menuText, {
        reply_markup: {
          keyboard: [['1', '2', '3'], ['1 & 2', '1 & 3', '2 & 3'], ['All three! ✨', '📋 Show my full menu']],
          resize_keyboard: true,
        },
      });
      return;
    ...
    if (selectionMap[text] && state.lastMenuTasks.length) {
      ...
      const response = await askHowl(`going to do: ${taskNames}`, 'React encouragingly, dramatically. Short.');
      await bot.sendMessage(chatId, response);
      return;
    }
    ...
    if (text === '➕ Add a task') {
      ...
      await bot.sendMessage(chatId, 'Which charter shall receive this new ambition?', {
        reply_markup: { keyboard: [['🧠 Grow & Build'], ['✍️ Create & Express'], ['🌿 Slow & Nourish']], resize_keyboard: true, one_time_keyboard: true },
      });
      return;
    }
    ...
    if (text === '📊 My progress') {
      const balance = getCharterBalance();
      const context = `Progress: Total=${state.history.length}. Grow=${balance.grow}, Create=${balance.create}, Nourish=${balance.nourish}. Dramatic Howl progress report with nudge toward neglected charter.`;
      const response = await askHowl('show progress', context);
      await bot.sendMessage(chatId, response);
      return;
    }

    const balance = getCharterBalance();
    const response = await askHowl(text, `Balance: Grow=${balance.grow}, Create=${balance.create}, Nourish=${balance.nourish}.`);
    await bot.sendMessage(chatId, response);
...
cron.schedule('0 8 * * *', () => sendDailyCheckin());
